...
// ✅ FIXED: Describes actual task — no vague income promise
const SLOGAN = "Answer surveys, label data & rate AI content";
...
async function startAnimationSequence() {
    ...
    // Phase 9: Show task description + loading + version
    await showTaskDesc();
    await sleep(375);
...
function flashEffect() {
    return new Promise(resolve => {
        ...
        setTimeout(() => {
            flash.style.transition = 'opacity 50ms ease';
...
function showIconGlow() {
    return new Promise(resolve => {
        ...
        glow.style.transition = 'all 150ms ease-out';
...
function showIconOuter() {
    return new Promise(resolve => {
        ...
        outer.style.transition = 'all 225ms cubic-bezier(0.34, 1.56, 0.64, 1)';
...
function bounceIcon() {
    return new Promise(resolve => {
        ...
        outer.style.transition = 'all 75ms ease-out';
        outer.style.transform = 'scale(1.08)';

        setTimeout(() => {
            outer.style.transition = 'all 75ms ease-in-out';
            outer.style.transform = 'scale(1)';
            setTimeout(resolve, 75);
        }, 75);
    });
...
function showTagline() {
    return new Promise(resolve => {
        ...
        tagline.style.transition = 'all 150ms ease-out';
...
// ================== PHASE 8.5: SHOW TASK DESCRIPTION ==================
// ✅ NEW: Fades in the task type label before the spinner appears
function showTaskDesc() {
    return new Promise(resolve => {
        const taskDesc = document.getElementById('taskDesc');
        if (!taskDesc) { resolve(); return; }

        taskDesc.style.transition = 'opacity 300ms ease';
...
function showLoadingAndVersion() {
    return new Promise(resolve => {
        ...
        loading.style.transition = 'opacity 100ms ease';
        loading.style.opacity = '1';

        version.style.transition = 'all 100ms ease-out';
...
function skipAnimations() {
    ...
    const taskDesc = document.getElementById('taskDesc');
    if (taskDesc) taskDesc.style.opacity = '1';
